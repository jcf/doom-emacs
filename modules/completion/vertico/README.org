...
=SPC b b=/=SPC ,= support changing the workspace you're selecting a buffer from
...
or the last workspace by typing =0 SPC=. =SPC f f=/=SPC .= support exported to a
=wdired= buffer using =C-c C-e=.
...
Doom has some builtin [[https://github.com/oantolin/orderless#style-dispatchers][style dispatchers]] for more finegrained filtering. You can
further specify each space separated input in the following ways:
| Input            | Description                                |
|------------------+--------------------------------------------|
